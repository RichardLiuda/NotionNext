...
function filterPostBlocks(id, blockMap, slice) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (
      ...
      //   const newUrl = `https://file.notion.so/f/f/b1106196-e2ad-4338-8aaf-780cd0a84a1b/30b8401d-b0ce-47fc-bd7b-9c6d43f0d874/avif_peak.avif?table=block&id=12a69776-e204-80a4-a72a-dc9be7e702ce&spaceId=b1106196-e2ad-4338-8aaf-780cd0a84a1b`
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const end = new Date().getTime()
    console.log(
      `[API<<--响应] 耗时:${end - start}ms Fetching missing blocks count:${
        ids.length
      } `
    )
